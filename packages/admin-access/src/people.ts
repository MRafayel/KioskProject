 ...
 * account holds the role it was created with, and changing that stays a CLI
 * operation performed by somebody with a database credential. A capability that
 * could turn an Operator into an Admin would make every other boundary in this
 * package a formality.
 *
 * **Nothing here enrols a key on somebody else's behalf.** WebAuthn will not
 * allow it: enrolment needs the person and their device in the same place. What
 * an Admin can do is authorise one enrolment ceremony on one named account, for
 * fifteen minutes, once — which is what `issueEnrollmentTicketBodySchema` is.
 *
 ...
 *
 * Fifteen minutes, because the ceremony it authorises is something two people do
 * together while one of them is on the phone to the other. A ticket that
 * outlived the conversation would be a credential sitting in a chat log.
 */
export const ENROLLMENT_TICKET_TTL_MILLISECONDS = 15 * 60 * 1000;

/**
 * The code itself: 256 bits, base64url, generated server-side and shown once.
 *
 * Bounded here so a redemption attempt with something that cannot be a code is
 * refused before it reaches a database lookup.
 */
...
/**
 * Asking for a ticket that lets one Operator enrol their first security key.
 *
 * The body carries no code, no account name and no role — only the reason. The
 * account is in the path, the code is generated by the server, and everything
 * about what the ticket may do is fixed by the server rather than requested:
 * one account, fifteen minutes, one ceremony, and only for an account that has
 * no usable authenticator yet.
 *
 * That last constraint is what stops this from being an impersonation tool
 * against a working account. A person who already holds a key gets a
 * replacement through `authenticator.manage.self`; if they have lost every key
 * they hold, that is a recovery, and recovery is the sealed offline break-glass
 * procedure — deliberately not something anybody can start from a browser.
 */
export const issueEnrollmentTicketBodySchema = z.object({ reason: recoveryReasonSchema }).strict();

export type IssueEnrollmentTicketBody = z.infer<typeof issueEnrollmentTicketBodySchema>;

/**
 * The one and only time the code is returned.
 *
 * It is not stored in a readable form — the database holds a peppered digest —
 * so this response cannot be reissued, re-read or recovered. The panel shows it
 * once and says so.
 */
export const enrollmentTicketResponseSchema = z.object({
  ...
  /** Shown once, never persisted in readable form, never written to a log. */
  enrollmentCode: enrollmentCodeSchema,
  expiresAt: z.string().datetime(),
  /**
   * Restates the boundary in the response, as the operations responses do. A
   * ticket authorises an enrolment ceremony; it is not a session, it carries no
   * capability, and redeeming it does not sign anybody in.
   */
  grantsSession: z.literal(false)
});

export type EnrollmentTicketResponse = z.infer<typeof enrollmentTicketResponseSchema>;

export const redeemEnrollmentTicketBodySchema = z
  .object({ enrollmentCode: enrollmentCodeSchema })
  .strict();

export type RedeemEnrollmentTicketBody = z.infer<typeof redeemEnrollmentTicketBodySchema>;
...
export const adminPersonSchema = z.object({
  ...
  usableAuthenticators: z.number().int().nonnegative(),
  minimumAuthenticators: z.number().int().positive(),
  ...
  /**
   * Unredeemed, unexpired enrollment tickets. Shown to everybody who can see
   * this section, because a ticket outstanding on an account is exactly the
   * thing a second pair of eyes should notice.
   */
  liveEnrollmentTickets: z.number().int().nonnegative(),
  enrollmentTicketExpiresAt: z.string().datetime().nullable()
});
