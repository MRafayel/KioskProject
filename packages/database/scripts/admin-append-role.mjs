...
export async function runAppendRoleCommand(policy, command) {
  ...
  const session = new AppendRoleSession(policy, client);
  try {
    switch (command) {
      case "provision":
...
class AppendRoleSession {
  /** @param {AppendRolePolicy} policy @param {pg.Client} client */
  ...
  async disable() {
    ...
    await client.query(`ALTER ROLE ${quoteIdentifier(policy.role)} NOLOGIN`);
    process.stdout.write(
